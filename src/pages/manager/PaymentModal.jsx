...
export default function PaymentModal({ amount, onClose, onSuccess }) {
  ...
  const validateExpiry = (exp) => {
    if (!/^\d{2}\/\d{2}$/.test(exp)) return false; // Must be in MM/YY format
  ...
  const handlePayment = () => {
    if (!amount || Number(amount.trim()) <= 0) {
      toast.error("Enter valid amount greater than 0");
      return;
    }

    const cleanCard = card.replace(/\s/g, ""); // remove whitespaces globally

    // CARD length check
    if (!/^\d{16}$/.test(cleanCard)) {
      toast.error("Card must be exactly 16 digits");
      return;
    }

    // EXPIRY
    if (!validateExpiry(expiry)) {
      toast.error("Expiry must be valid and not in past");
      return;
    }
